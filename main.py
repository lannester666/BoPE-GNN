...
# count_label_homo(dataset, order=3, dataname=args.dataset)
# exit()
...
print(f"total parameter number:{total_params}")
model.train()
print('MODEL:', model)
model.tebd_type = args.tebd_type
if args.method == 'gcnt' and args.treduc_save:
    model.treduc_save(dataset, args.dataset)
    exit()
for run in range(args.runs):
    split_idx = split_idx_lst[run]
    if args.re_split:
    ...
    best_val = float('-inf')
    counter = 0
    trains = copy.deepcopy(dataset.label[split_idx['train']])
    vals = copy.deepcopy(dataset.label[split_idx['valid']])
    tmp = copy.deepcopy(dataset.label[split_idx['test']])
    for epoch in range(args.epochs):
        ...
        t3 = time.time()
        temp = class_acc( out[train_idx], dataset.label[train_idx])
        val_acc = class_acc( out[val_idx], dataset.label[val_idx])
        model.val_acc = torch.tensor(val_acc).to(device)
        model.train_acc = torch.tensor(temp).to(device)
        if epoch % 100 == 0:
    ...
    if args.paint_tsne:
        t_sne_analyis(args,model, dataset, features=dataset.graph['node_feat'], split = split_idx)
    
    row, col = dataset.graph['edge_index']
    N = dataset.graph['num_nodes']
    adj = SparseTensor(row=row, col=col, sparse_sizes=(N, N)).to(device)
