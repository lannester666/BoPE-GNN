...
def load_nc_dataset(dataname, sub_dataname='', args=None,run=0):
    ...
    if dataname == 'twitch-e':
        ...
        dataset = load_planetoid_dataset(dataname,run=run, args=args)
...
def load_planetoid_dataset(name,args=None, run=0):
    # torch_dataset = Planetoid(root=f'{DATAPATH}/Planetoid',
    #                           name=name)
    # data = torch_dataset[0]
    adj, features, labels, idx_train, idx_val, idx_test = load_data_new(name, run, normfeat=False) 
