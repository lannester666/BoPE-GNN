...
class Logger(object):
    ...
    def print_statistics(self, run=None, stage=None):
        if run is not None:
            ...
            if stage == 0:
                self.ori_results.append(result[argmax, 2])
            if stage is not None:
                print(f'Stage 0 Test result: {self.ori_results[run]}')
            ...
            for i in range(len(self.results)):
                r= 100 * torch.tensor(self.results[i])
                train1 = r[:, 0].max().item()
                valid = r[:, 1].max().item()
